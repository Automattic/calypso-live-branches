...
var debug = require( 'debug' )( 'clb-repo' );
	...
	this.init( function( err ) {
		...
		( function sync() {
			...
			repo._sync( function( err ) {
				if ( err ) console.error( 'Ignoring sync error: ', err );
				repo.syncing = false;
				repo.emit( 'synced' );
...
MirrorRepository.prototype.exec = function( command, callback ) {
	...
	if ( ! fs.existsSync( directory ) ) {
		return callback( new Error( 'Repository directory ' + directory + ' does not exist while trying to run: ' + command ) );
	}
	exec( command, {
		cwd: directory
