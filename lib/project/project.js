...
Project.prototype.getSocketPath = function( branchName ) {
	if ( process.env.SOCKET_DIR ) {
		return path.join( process.env.SOCKET_DIR, this.name, branchName + '.sock' );
	}

	return path.join( this.destination, 'sockets', branchName+'.sock' );
};
...
Project.prototype.cleanupBranch = function( branchName ) {
	...
	try {
		...
		if ( fs.existsSync( branch.getDirectory() ) ) {
			debug( 'Failed cleaning up branch ' + branch.name );
		}
	} catch( err ) {
		return err;
	}
...
Project.prototype.startJanitor = function() {
	...
	exec( 'mkdir -p ' + shellescape( [ emptyDir ] ), function() {
		...
		exec( 'rsync -aqr --delete --ignore-errors ' +
				shellescape( [ emptyDir+'/', self.graveyard+'/' ] ),
