		...
		fs.stat(destinationDir, function(err) {
			...
			mkdirp(destinationDir, function(err) {
				...
				exec('git clone '+repo.getDirectory()+' '+destinationDir, function(err) {
					if(err) return callback(err, destinationDir);
					branch.updateConfig(function(err) {
						if(err) return callback(err, destinationDir);
...
Branch.prototype.updateConfig = function(callback) {
	exec('git config --add remote.origin.fetch "+refs/pull/*/head:refs/remotes/origin/pr/*"; git fetch origin', {
		cwd: this.getDirectory()
	}, callback);
};
...
Branch.prototype.exists = function(callback) {
	var branchName = this.name;
	var prMatches = branchName.match(/pr\/([0-9]+)/);
	if(prMatches && prMatches[1]) {
		branchName = 'pull/'+prMatches[1]+'/head';
	}
	var repo = this.repository;
	// check if ref exists in the mirror repository
	exec('git ls-remote '+repo.getDirectory()+' '+branchName, function(err, stdout) {
