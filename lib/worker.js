	...
	hub.on('init', function(data, sender, callback) {
		...
		resetFile(logPath, function() {
			...
			if (socketPath.length >= 103) {
				log('WARNING: Socket path is longer than UNIX_PATH_MAX on OS X (104), this might cause some problems');
			}
			...
			currentBranch.checkout(function(err, destination) {
				...
				log('branch '+branchName+' checkouted in '+destination);
				...
				exec('npm run clean', { cwd: branchDestination }, function (err) {
					...
					patchPackageJSON(destination, config, function(err) {
						...
						resetFile(socketPath, function() {
							...
							log('net.Server.prototype.listen patched');
							...
							// close and reopen log stream to pipe the `npm install` process output
							log = debug;
							outputStream.end();
							outputStream = fs.createWriteStream(logPath, {flags: 'a'});
							installProcess.stdout.pipe(outputStream);
							installProcess.stderr.pipe(outputStream);
						});
					});
				});
			});
