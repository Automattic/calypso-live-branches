...
module.exports = function(config) {
	...
	hub.on('init', function(data, sender, callback) {
		...
		resetFile(logPath, function() {
			...
			// patch console calls
			var logger = new console.Console(outputStream, outputStream);
			...
			currentBranch.checkout(function(err, destination) {
				...
				patchPackageJSON(destination, config, function(err) {
					if(err) return callback(serializeError(err));
					console.log('Cleaning before install...');
					var cleanProcess = exec('npm run clean', { cwd: branchDestination }, function() {
					...
					cleanProcess.stdout.on('data', function(chunk) {
						outputStream.write(chunk);
					});
					cleanProcess.stderr.on('data', function(chunk) {
						outputStream.write(chunk);
					});
				});
			});
