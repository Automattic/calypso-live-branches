...
module.exports = function(proxyManager) {
	...
	function proxyHTTP(req, res, next) {
		...
		if (req.query.status) {
			return proxyManager.checkUpdated(branchName, function() {
				res.send({ branch: branchName, status: proxyManager.getStatus(branchName) });
			});
		}
		...
		if (proxyManager.isBooting(branchName)) {
			...
			proxyManager.checkUpdated(branchName, function() { // ignore update errors
