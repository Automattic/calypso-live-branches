...
module.exports = function(proxyManager, appDefaultConfig) {
	...
	function proxyHTTP(req, res, next) {
		if ('list_branches' in req.query) {
			return res.json( proxyManager.listBranches() );
		}
		...
		if ('reboot' in req.query) {
			proxyManager.stopServingBranch(branchName, true);
		...
		if (!proxyManager.isUp(branchName)) {
			if ( proxyManager.hasErrored( branchName ) ) {
				serveBootPage(req, res, 'Branch has encountered an error. Retrying in 20s', options);
				return;
			}
		...
		if (proxyManager.isBooting(branchName)) {
			...
			proxyManager.checkUpdated(branchName, function(err, mustRestart) {
				...
				proxyManager.proxyRequest(branchName, req, res, function(err) {
					if ( err ) {
						debug( 'Proxying request to ' + branchName + ' failed.', err );
						// retry on error
						return serveBootPage(req, res, 'Rebooting branch...', options);
					}
				} );
