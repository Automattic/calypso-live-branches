...
function serveBootPage(req, res, message) {
    ...
    if(isHTML(req)) {
		res.render('boot', {
			message: message,
			config: {
				logUrl: req.path + '?branch=' + getBranchName(req) + '&log=1&format=html',
...
module.exports = function(proxyManager) {
	...
	function proxyHTTP(req, res, next) {
		...
		if (req.query.log) {
			...
			return serveLogFile(req, res, branchOutput, req.query);
		}
		if (req.query.reboot) {
		...
		if (proxyManager.isBooting(branchName)) {
			...
			proxyManager.checkUpdated(branchName, function(err, mustRestart) {
				if (err) {
					return serveBootPage(req, res, err.message);
				}
				if (mustRestart) {
					return serveBootPage(req, res, 'Rebooting branch...');
				}
