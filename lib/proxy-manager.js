...
ProxyManager.prototype.stopServingBranch = function(branchName, cleanup) {
	var proxy = this.proxies[branchName];
	var worker = this.workers[branchName];
	console.log('stopping branch '+branchName);
	if(proxy) {
		proxy.close();
	...
	if ( cleanup ) {
		this.project.cleanupBranch(branchName);
	}
};
...
ProxyManager.prototype.listBranches = function() {
	...
	if ( this.calypsoLiveConfig.readyEvent ) {
		readyBranches = activeBranches.filter( function( branchName ) { return self.bootStatuses[branchName] === self.calypsoLiveConfig.readyEvent; } );
	} else {
...
ProxyManager.prototype.markErrored = function(branchName) {
	var self = this;
	debug('Branch ' + branchName + ' has errored. It will be cleaned up an ready to boot again in 20s.' );
	this.bootStatuses[branchName] = "ERROR";
	// Remove branch after 1 minute to give it a chance to reboot
	setTimeout( function() {
